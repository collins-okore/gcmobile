...
import { SECURITY_GUARD_URLS } from "./apiUrls";
...
const securityGuardService = {
  ...
  getAllSecurityGuards: async (params?: PaginationParams) => {
    ...
    const url = `${SECURITY_GUARD_URLS.GET_ALL}${
  ...
  getSecurityGuardById: async (id: string) => {
    const response = await apiClient.get<SecurityGuard>(
      SECURITY_GUARD_URLS.GET_ONE(id)
    );
    return response.data;
  },
  ...
  deleteSecurityGuard: async (id: string) => {
    const response = await apiClient.delete(
      SECURITY_GUARD_URLS.DELETE(id)
    );
    return response.data;
  },

  // Get security guard by user ID
  getSecurityGuardByUserId: async (userId: string) => {
    try {
      // Use the GET_ALL endpoint with a filter for user ID
      const response = await apiClient.get<{ data: SecurityGuard[]; meta: PaginationMeta }>(
        `${SECURITY_GUARD_URLS.GET_ALL}?filters[user][id][$eq]=${userId}`
      );
      
      // Return the first security guard found for this user ID
      if (response.data.data && response.data.data.length > 0) {
        return response.data.data[0];
      }
      return null;
    } catch (error) {
      console.error("Error fetching security guard by user ID:", error);
      return null;
    }
  },
};
